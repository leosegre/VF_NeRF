...
class BaseDistribution(nn.Module):
    ...
    def sample(self, num_samples=1, sample_scale=1, **kwargs):
        ...
        """
        z, _ = self.forward(num_samples, sample_scale=sample_scale, **kwargs)
        return z
...
class DiagGaussian(BaseDistribution):
    ...
    def forward(self, num_samples=1, sample_scale=1):
        eps = torch.randn(
            (num_samples,) + self.shape, dtype=self.loc.dtype, device=self.loc.device
        )
        if self.temperature is None:
            log_scale = self.log_scale * sample_scale
        else:
